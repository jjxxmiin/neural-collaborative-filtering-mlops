...
@hydra.main(version_base=None, config_path="conf", config_name="neumf")
def main(cfg):
    ...
    gmf.load_state_dict(torch.load(cfg.train.gmf_checkpoint))
    
    mlp = MLP(cfg).to(cfg.train.device)
    mlp.load_state_dict(torch.load(cfg.train.mlp_checkpoint))
    
    neumf = NeuMF(cfg).to(cfg.train.device)
    neumf.load_pretrain_weights(gmf, mlp)
    
    criterion = nn.BCELoss()
    optimizer = optim.Adam(neumf.parameters(), 
    ...
    with mlflow.start_run():
        ...
        for e in range(cfg.train.epoch):
            ...
            hit_ratio, ndcg = evaluator(neumf, test_data)
            mlflow.log_metric('hit_ratio', hit_ratio, step=step)
            mlflow.log_metric('ndcg', ndcg, step=step)
            
            if train_loss < min_loss:
                min_loss = train_loss
                torch.save(neumf.state_dict(), cfg.train.best_checkpoint)
                
            torch.save(neumf.state_dict(), cfg.train.latest_checkpoint)
