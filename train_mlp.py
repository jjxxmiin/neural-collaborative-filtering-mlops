...
def main(cfg):
    ...
    with mlflow.start_run():
        ...
        for e in range(cfg.train.epoch):
            ...
            hit_ratio, ndcg = evaluator(mlp, test_data)
            mlflow.log_metric('hit_ratio', hit_ratio, step=step)
            mlflow.log_metric('ndcg', ndcg, step=step)
            
            if train_loss < min_loss:
                min_loss = train_loss
                torch.save(mlp.state_dict(), cfg.train.best_checkpoint)
                ml_torch.log_model(mlp, 'ml_model')
                
            torch.save(mlp.state_dict(), cfg.train.latest_checkpoint)
